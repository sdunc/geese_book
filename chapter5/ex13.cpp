...
int main()
{
  ...
  // set the game up by getting a seed from the user and then making a
  // vector will 4 random components using the randint() function
  // provided by std_lib_facilities.h
  cout << "Enter an integer seed for the random number generator: ";
  cin>>seed;
  for (int i=0; i<4; ++i)
    bandc.push_back(randint(seed));
